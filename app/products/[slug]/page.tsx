...
export default async function ProductPage({ params }: Props) {
  ...
  let detailRes;
  try {
    detailRes = await serverCallFuction('GET', `api/products/product-detail/${slug}`);
  ...
  const product: Product = {
    ...
    f_image: apiProduct.product.f_image ? `${apiProduct.product.f_image}` : '/assets/product/sanitory_pad.png',
    g_images: (apiProduct.product.g_images || []).map(img => `${img}`),
    specs: apiProduct.product.specs || {},
    category: apiProduct.category as Category || { id: 0, name: 'Uncategorized', slug: 'uncategorized' },
  ...
  return (
    <>
    {product ? (
      <ProductDetail product={product} attributes={attributes} variants={variants} />
    ) : (
